...
static int days=0;
  static int overAllTripsCounter=0;
  ...
  static double calculateFareAmountFromOriginToDestination(DirectionDetailsInfo directionDetailsInfo){
    ...
    // print("overalltrips :$overAllTripsCounter");
    // return double.parse(totalFareAmount.toStringAsFixed(1));

    if (selectedVehicleType == "Bike") {
      localCurrencyTotalFare= totalFareAmount * 50;
      totalFareAmount = ((localCurrencyTotalFare.truncate()) * 1.92);
      ...
      localCurrencyTotalFare = totalFareAmount * 50;
      totalFareAmount = ((localCurrencyTotalFare.truncate()) * 3.44);
   ...
   static double customerLoyalty() {

     for (days = 0; days <= 30; days++) {

       if (overAllTripsCounter > 25) {
         discount = totalFareAmount * 0.05;
         totalFareAmount = totalFareAmount - discount;
         return totalFareAmount;
       }
       if (overAllTripsCounter > 20) {
         discount = totalFareAmount * 0.03;
         totalFareAmount = totalFareAmount - discount;
         return totalFareAmount;
       }
       if (overAllTripsCounter > 15) {
         discount = totalFareAmount * 0.015;
         totalFareAmount = totalFareAmount - discount;
         return totalFareAmount;
       }
     }
     days=0;
     overAllTripsCounter = Provider.of<AppInfo>(context as BuildContext, listen: false).restartCount(overAllTripsCounter);
     return totalFareAmount;
   }
  static void calculateFare(DirectionDetailsInfo directionDetailsInfo) {
    calculateFareAmountFromOriginToDestination(directionDetailsInfo);
    customerLoyalty();
  ...
  static void readTripsKeysForOnlineUser(context){
    ...
    FirebaseDatabase.instance.ref().child("All Ride Requests").orderByChild("userName").equalTo(userModelCurrentInfo!.name).once().then((snap){
      if(snap.snapshot.value != null){
        ...
        overAllTripsCounter = keysTripsId.length;
